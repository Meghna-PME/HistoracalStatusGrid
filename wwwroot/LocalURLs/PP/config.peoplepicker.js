    ...
    .directive('uiPeople', ['uiPeopleConfig', '$q', '$injector', '$resource', '$timeout', function (uiPeopleConfig, $q, $injector, $resource, $timeout) {
        ...
        return {
            ...
            link: function (scope, elm, attrs, ngModel) {
                ...
                var pickerWidth = (attrs.ppWidth && attrs.ppWidth.match(/^[0-9][0-9]*px$/i)) ? attrs.ppWidth : '100%';
                ...
                models.userCollection.prototype.updateFromViewValue = function (value, scope) {
                    ...
                    $('#' + pickerId).addClass("k-input");
                    $('#' + pickerId).width('100%');
                    //$('#'+pickerId).height('auto');
                    //$('#'+pickerId).css({'height':'29px'});
                    ...
                    if (value && value.results instanceof Array) {
                        /*//Commented for enhancement 252 & 253
                        ...
                        //Start - Enhancement 252 & 253.
                        for (var i = 0; i < value.results.length; i++) {
                            var cu = value.results[i];
                            //update to remove GUID from security group names
                ...
                ngModel.$render = Function.createDelegate(scope, function () {
                    scope.isDisabled = attrs.ngDisabled ? scope.$eval(attrs.ngDisabled) : false;
                    //if(userModel._currentUsers.length >0)
                ...
                function initializePeoplePicker(scope) {
                    ...
                    schema.OnValueChangedClientScript = getusers;
                    //loading = false;
                ...
                function getusers(elementId, userKeys) {
                ...
                function setUserIdFromPickerChoice(elementId, userKeys) {
                    ...
                    if (loading) {
                        try {
                            if (userKeys.length > 0 && userKeys[userKeys.length - 1].DisplayText == undefined) {
                                var peoplePicker = SPClientPeoplePicker.SPClientPeoplePickerDict[elementId];
                                //get selected users and select the second user (index 1) to remove
                                var selectedUsers = peoplePicker.GetAllUserInfo();
                                var userToRemoveKey = selectedUsers[userKeys.length - 1].Key;
                                var resovledListElmId = peoplePicker.ResolvedListElementId;
                                var elementToRemove = '';
                                $('#' + resovledListElmId).children().each(function (index, element) {
                                    if (element.id.startsWith(elementId + '_' + userToRemoveKey + '_ProcessedUser')) {
                    ...
                    //alert(_spPageContextInfo.serverRequestPath.indexOf('employeeprofileinformation.aspx'));
                    //(!_spPageContextInfo.serverRequestPath.includes('employeeprofileinformation.aspx'))
                    /*if (userKeys.length > 0) {
                        //if ((userKeys[userKeys.length - 1].DisplayText == undefined) || (userKeys.length > 0 && !BizUserProfieFactoryService.IsValidUser(userKeys[userKeys.length - 1].Key)) && (_spPageContextInfo.serverRequestPath.indexOf('employeeprofileinformation.aspx') <= 0) && (_spPageContextInfo.serverRequestPath.toLowerCase().indexOf('user-profile-information.aspx') <= 0)) {
                        alert("Selected user's profile is not available/active, Please contact system administrator.");
                        var peoplePicker = SPClientPeoplePicker.SPClientPeoplePickerDict[elementId];
                        //get selected users and select the second user (index 1) to remove
                        var selectedUsers = peoplePicker.GetAllUserInfo();
                        var userToRemoveKey = selectedUsers[userKeys.length - 1].Key;
                    ...
                    if (!this.AllowMultipleUsers && this.TotalUserCount > 1) {
                        alert('You can only enter one name.');
